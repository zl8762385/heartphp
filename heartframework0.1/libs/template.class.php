...
class template {
	 ...
	 * 编译
	 */	
	private function compile($newfile) {
	...
	private function template_replace($str) {
		...
		$header_comment = "Create On##".date('Y-m-d H:i:s')."|Compiled from##".$this->template_path.$this->template_name;
		$template = "<? if(!defined('IS_HEARTPHP')) exit('Access Denied');/*{$header_comment}*/?>\r\n$str";

		//$this->create_dir($this->get_tpl_path()); //留着明天做  当编译的时候需要创建递归目录 这样多级方便管理
		$return_c_file = $this->c_file('zhangliang', $template);
	 ...
	 * 写入文件
	 */		
	private function c_file($filename, $content) {
		if(empty($filename)) $this->error("{$filename} Creation failed");
		$filename = $this->template_c.$filename.$this->tpl_compile_suffix;
